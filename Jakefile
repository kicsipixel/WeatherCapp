...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "WeatherCapp.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("WeatherCapp");
    task.setIdentifier("com.yourcompany.WeatherCapp");
    task.setVersion("1.0");
    task.setAuthor("Your Company");
    task.setEmail("feedback @nospam@ yourcompany.com");
    task.setSummary("WeatherCapp");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "WeatherCapp.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "WeatherCapp.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
